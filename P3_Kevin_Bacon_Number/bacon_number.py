...
class BaconNumberCalculator:
    ...
    def _reconstructPath(self, startActor, endActor, previousPath):
        """
        Reconstruct the path from startActor to endActor. 
        ...
        previousPath: dictionary of tuples
            The dictionary that stores the previous actor and related movie for each actor.
        ...
        List[int, List[str]]
            ...
            [startActor, movie1, actor2, movie2, actor3, movie3,endActor]
        """
        path = [endActor]
        curActor = endActor
        
        while curActor != startActor:
            prevActor, movie = previousPath[curActor]
            path.append(movie)
            path.append(prevActor)
            curActor = prevActor
            
        path.reverse()
        baconNumber = (len(path) - 1) / 2
        return [baconNumber, path]
    ...
    def calcBaconNumber(self, startActor, endActor) -> list[int | list[str]]:
        ...
        # Method implementation...
        # If one of the inputted actor in not in our graph. 
        if startActor not in self.adjList or endActor not in self.adjList:
            return [-1, []]
        if startActor == endActor: # If the starter actor is the end actor. 
            return [0, [startActor]]
        
        # Create a queue for BFS with only actor stored. 
        queue = deque([startActor])
        visited = set([startActor]) # Use a set for recording the visited actors. 
        
        # Create a dictionary to store the previous actor and the related movie
        # for reconstructing the path. 
        previousPass = {}
        
        while queue: 
            current = queue.popleft()
            for neighbor, movie in self.adjList[current].items():
                if neighbor not in visited:
                    queue.append(neighbor)
                    visited.add(neighbor)
                    # current -> movie -> neighbor
                    previousPass[neighbor] = (current, movie)
                    
                    if neighbor == endActor:
                        return self._reconstructPath(startActor, endActor, previousPass)
    ...
    def calcAvgNumber(self, startActor, threshold) -> float:
        ...
        while curDiff > threshold:
            ...
            bNum, _ = self.calcBaconNumber(startActor, actor)
...
% I worked with the following classmates: I worked alone on this project. 
